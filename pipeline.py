...
# Imports for the contrastive diffusion
from utils import JointAttnProcessor2_0, JEDI, JEDIOutput
...
class SD3JediPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    r"""
    Args:
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if jedi is not None and i < jedi.timestep_end and jedi.lr > 0:
                    with torch.enable_grad():
                        ...
                        loss = jedi.compute_loss()
                        loss.backward()

                        latents = jedi.grad_update(latents)
        ...
        if jedi is not None:
            return JEDIOutput(
                images=image,
                js_pos=torch.stack(js_pos_ls).mean().item(),
                js_neg=torch.stack(js_neg_ls).mean().item(),
                entropy=torch.stack(ent_ls).mean().item(),
            )
